...
This is the canonical shared handoff for `AGENTS.md`, `GEMINI.md`, and
`CLAUDE.md`; update this file rather than duplicating long-lived instructions
in the root wrappers.
...
## Current Multi-Arm Hardware Notes

Power stability matters for servo discovery. A recent failure where only IDs
`5` and `6` responded was traced to the servo power path going through two
power expanders. Plugging the power expander directly into the wall, rather
than through a second expander, restored all six servos with model `777`.

Assume a normal SO-101 servo ID order of `1,2,3,4,5,6` unless a specific arm is
documented otherwise. Arm A and Arm B are both currently documented with a base
rotation/base pitch physical order of `2,1,3,4,5,6`:
...
Do not assume the physical base joint order matches the usual SO-101 ID order
until the bus IDs and calibration endpoints have been verified.

Before bringing up Arm B, assign unique servo IDs one servo at a time. Use
`scripts/set_feetech_id.py` only with a single physical servo connected to the
bus, then verify with `probe_feetech`/`list_connected_robots` before adding the
next servo.
...
`robot_id`; calibration files are keyed by `robot_id`.

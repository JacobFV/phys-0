...
"""
Records a dataset via teleoperation.

Requires: pip install 'lerobot[core_scripts]'
...
def record_loop(
    ...
    if isinstance(teleop, list):
        ...
        teleop_arm = next(
            (t for t in teleop if isinstance(t, (so_leader.SO100Leader | so_leader.SO101Leader | koch_leader.KochLeader | omx_leader.OmxLeader))),
            None,
        )
        if not (teleop_arm and teleop_keyboard and len(teleop) == 2 and robot.name == "lekiwi_client"):
            raise ValueError("For multi-teleop, the list must contain exactly one KeyboardTeleop and one arm teleoperator. Currently only supported for LeKiwi robot.")
    ...
    while timestamp < control_time_s:
        ...
        if sleep_time_s < 0:
            logging.warning(f"Record loop is running slower ({1 / dt_s:.1f} Hz) than target FPS ({fps} Hz).")
...
def record(
    ...
    try:
        ...
        if not cfg.dataset.streaming_encoding:
            logging.info("Streaming encoding is disabled. Consider --dataset.streaming_encoding=true for faster episode saving.")
